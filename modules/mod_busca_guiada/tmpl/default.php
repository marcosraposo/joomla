			...
			<div class="row">
                ...
                <div class="col-9 consultacontainer">
					...
					<div class="option margin">
                        <div class="chevron volta" onclick="javascript:resultadoBuscaGuiada('portal-da-transparencia','1','0','avancar')";><</div>
                        Escolha uma opção
                    </div>
						...
						foreach($buscaGuiadaServico as $retorno){
							foreach($retorno->categoria as $botoes){
								if(is_object($botoes)){ 
									...
									<?php }else{ ?>
										<div class="box" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->id; ?>','<?php echo "portal-da-transparencia/".$botoes->id;?>','<?php echo $nivel+1; ?>','','<?php echo $botoes->title; ?>', 'avancar')";>
											<img src="../templates/portalTransparencia/images/folha.svg">
											<?php echo $botoes->title; ?>
										</div>
							  <?php }
										...
										if(is_object($botoes)){
											if($botoes->exibe){
												if(!empty($botoes->sv) && $botoes->sv != ""){ ?>
													<div class="box" style="<?php echo tamanhoFontBotao($botoes->title);?>" onClick="window.location.href='../index.php/gestao-orcamentaria/resultado-pdf?/id=<?php echo $botoes->id; ?>&MOD=<?php echo $botoes->sv; ?>&niveis=<?php echo $tituloNiveis; ?>'" >
														<img src="../templates/portalTransparencia/images/folha.svg">
														<?php echo $botoes->title; ?>
													</div>
												<?php 
												...
												<?php }else{ ?>
													<div class="box" style="<?php echo tamanhoFontBotao($botoes->title);?>" onclick="javascript:resultadoBuscaGuiada('<?php echo $botoes->path; ?>','<?php echo $origem."/".$botoes->path; ?>','<?php echo $nivel+1; ?>','<?php echo $botoes->params; ?>','<?php echo $tituloNiveis."/".$botoes->title; ?>','avancar')";>
														<img src="../templates/portalTransparencia/images/folha.svg">
														<?php echo $botoes->title; ?>
													</div>
											<?php }
